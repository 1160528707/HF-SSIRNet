...
def memory_querying_responding(query, key, value, mask=None, dropout=None, topk=32):
    ...
    if mask is not None:
        scores = scores.masked_fill(mask == 0, float('-inf'))
    selected_scores, idx = scores.topk(1)
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, layer_past=None):
        ...
        if layer_past is not None and not (layer_past.shape[2] == key.shape[1] > 1):
            past_key, past_value = layer_past[0], layer_past[1]
            assert past_key.shape[-1] == key.shape[-1], \
                f"past_key 维度不匹配: {past_key.shape} vs {key.shape}"
            key = torch.cat((past_key, key), dim=1)
            value = torch.cat((past_value, value), dim=1)
            present = torch.stack([key, value])
...
class BaseCMN(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        # self.embeddings = Embeddings(tgt_vocab, self.d_model)
        ...
        nn.init.normal_(self.memory_matrix, 0, 1 / args.cmm_dim)      #需要改
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        if len(state) == 0:
            ys = it.unsqueeze(1)
            # 动态获取当前 batch_size
            batch_size = fc_feats_ph.size(0)
            # 初始化 past 的维度为 [num_layers*2, batch_size, 0, d_model]
            past = [
                fc_feats_ph.new_zeros(self.num_layers * 2, batch_size, 0, self.d_model),
                fc_feats_ph.new_zeros(self.num_layers * 2, batch_size, 0, self.d_model)
            ]
        else:
